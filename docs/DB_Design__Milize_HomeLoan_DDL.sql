...
CREATE TABLE `p_applicant_persons` (
  ...
  `rel_to_applicant_a` char(1) DEFAULT NULL COMMENT '連帯債務者　申込者に対して続柄',
...
CREATE TABLE `p_bank_api_send_histories` (
  ...
  `set_num` bigint unsigned DEFAULT NULL COMMENT 'SET番号',
  `request_number` bigint unsigned DEFAULT NULL COMMENT '申込認識番号',
  `pair_loan_id` bigint unsigned DEFAULT NULL COMMENT 'ペアローン相手ID',
  `request_body` varchar(255) DEFAULT NULL COMMENT 'リクエストボディ',
  `status_code` varchar(255) DEFAULT NULL COMMENT 'ステータスコード',
  `response_body` varchar(255) DEFAULT NULL COMMENT 'レスポンスボディ',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '作成日付',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日付',
  PRIMARY KEY (`id`) USING BTREE,
...
CREATE TABLE `p_examine_status_histories` (
  ...
  `status_origin` varchar(255) DEFAULT NULL COMMENT '審査状態源',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '作成日付',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日付',
