...
async def get_type_and_id(db: DB, viewed_account_id, viewed_account_type):
    if viewed_account_type == "User":
        user = await db.fetch_one(f"SELECT id FROM mortgage_staging_v3.c_users WHERE old_id = {viewed_account_id}")
        user_id = None
        if user:
            user_id = user["id"]
    ...
    if viewed_account_type == "SSalePerson":
        user = await db.fetch_one(
            f"SELECT id FROM mortgage_staging_v3.s_sales_persons WHERE old_id = {viewed_account_id}"
        )
        user_id = None
        if user:
            user_id = user["id"]
    ...
    if viewed_account_type == "Manager":
        user = await db.fetch_one(f"SELECT id FROM mortgage_staging_v3.s_managers WHERE old_id = {viewed_account_id}")
        user_id = None
        if user:
            user_id = user["id"]
...
async def translate_c_messages(db: DB):
    ...
    JOIN
        mortgage_staging_v3.c_messages as nm
        ON
        nm.old_id = om.id
    """
    for message in await db.fetch_all(sql):
        ...
        await db.execute(
            f"""UPDATE c_messages SET viewed = '{json.dumps(v_list, ensure_ascii=False)}' WHERE id = {message["id"]}"""
        )
