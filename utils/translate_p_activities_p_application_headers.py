...
async def translate_p_activities_p_application_headers(db: DB, p_application_header_id):
    ...
    FROM
        mortgage_staging_v3.p_application_headers as h
    LEFT JOIN
        ...
        if "planned_cohabitant" in parameters:
            ...
            if operator_type == 1:
                user = await db.fetch_one(
                    f"SELECT id FROM mortgage_staging_v3.c_users WHERE old_id = {PApplicationHeaderCreateData['owner_id']};"
                )
                if user:
                    operator_id = user["id"]
            if operator_type == 2:
                sales_person = await db.fetch_one(
                    f"SELECT id FROM mortgage_staging_v3.s_sales_persons WHERE old_id = {PApplicationHeaderCreateData['owner_id']};"
                )
                if sales_person:
                    operator_id = sales_person["id"]
            if operator_type == 3:
                manager = await db.fetch_one(
                    f"SELECT id FROM mortgage_staging_v3.s_managers WHERE old_id = {PApplicationHeaderCreateData['owner_id']};"
                )
                if manager:
    ...
    for PApplicationHeaderUpdateData in PApplicationHeadersUpdateData:
        ...
        if "planned_cohabitant" in parameters:
            ...
            if operator_type == 1:
                user = await db.fetch_one(
                    f"SELECT id FROM mortgage_staging_v3.c_users WHERE old_id = {PApplicationHeaderUpdateData['owner_id']};"
                )
                if user:
                    operator_id = user["id"]
            if operator_type == 2:
                sales_person = await db.fetch_one(
                    f"SELECT id FROM mortgage_staging_v3.s_sales_persons WHERE old_id = {PApplicationHeaderUpdateData['owner_id']};"
                )
                if sales_person:
                    operator_id = sales_person["id"]
            if operator_type == 3:
                manager = await db.fetch_one(
                    f"SELECT id FROM mortgage_staging_v3.s_managers WHERE old_id = {PApplicationHeaderUpdateData['owner_id']};"
                )
                if manager:
                    operator_id = manager["id"]
            planned_cohabitant = parameters.get("planned_cohabitant", [])
                    ...
                    if p_referral_agencies:
                        if p_referral_agencies["sale_agent_id"]:
                            org_c = await db.fetch_one(
                                f"""SELECT id FROM mortgage_staging_v3.s_sales_company_orgs WHERE code = '{p_referral_agencies["sale_agent_id"]}';"""
                            )
                        ...
                        if p_referral_agencies["store_id"]:
                            org_b = await db.fetch_one(
                                f"""SELECT id FROM mortgage_staging_v3.s_sales_company_orgs WHERE code = '{p_referral_agencies["store_id"]}';"""
                            )
                        ...
                        if p_referral_agencies["exhibition_id"]:
                            org_e = await db.fetch_one(
                                f"""SELECT id FROM mortgage_staging_v3.s_sales_company_orgs WHERE code = '{p_referral_agencies["exhibition_id"]}';"""
                            )
