...
async def translate_p_uploaded_files(db: DB):
    ...
    for old_file_info in old_files_info:
        ...
        if owner_type == 1:
            user = await db.fetch_one(
                f"SELECT id FROM mortgage_staging_v3.c_users WHERE old_id = {old_file_info['role_id']};"
            )
            if user:
                owner_id = user["id"]
        if owner_type == 2:
            sales_person = await db.fetch_one(
                f"SELECT id FROM mortgage_staging_v3.s_sales_persons WHERE old_id = {old_file_info['role_id']};"
            )
            if sales_person:
                owner_id = sales_person["id"]
        if owner_type == 3:
            manager = await db.fetch_one(
                f"SELECT id FROM mortgage_staging_v3.s_managers WHERE old_id = {old_file_info['role_id']};"
            )
            if manager:
                owner_id = manager["id"]
        p_application_header_id = None
        p_application_header = await db.fetch_one(
            f"SELECT id FROM mortgage_staging_v3.p_application_headers WHERE old_id = {old_file_info['old_record_id']};"
        )
    ...
    for old_file_info in old_files_info:
        ...
        p_applicant_persons = await db.fetch_one(
            f"SELECT id, p_application_header_id, type FROM mortgage_staging_v3.p_applicant_persons WHERE old_id = {old_file_info['old_record_id']};"
        )
    ...
    for old_file_info in old_files_info:
        ...
        p_borrowing = await db.fetch_one(
            f"SELECT id, p_application_header_id FROM mortgage_staging_v3.p_borrowings WHERE old_id = {old_file_info['old_record_id']};"
        )
    ...
    for data in new_data:
        await db.execute(utils.gen_insert_sql("mortgage_staging_v3.p_uploaded_files", data))
