...
async def translate_p_archive_files(db: DB):
    ...
    INNER JOIN
        mortgage_staging_v1.s_sales_persons as sp
        ON
    ...
    for item in old_basic_info:
        ...
        sql = f"SELECT CONVERT(sor.s_sales_company_org_id,CHAR) AS s_sales_company_org_id, sor.role FROM mortgage_staging_v1.s_sales_person_s_sales_company_org_rels as sor WHERE sor.s_sales_person_id = {item['s_sales_person_id']};"
        orgs = await db.fetch_one(sql)
        sql = f"""
        WITH RECURSIVE child AS (
        SELECT id, pid, category FROM mortgage_staging_v1.s_sales_company_orgs WHERE id = {orgs['s_sales_company_org_id']}
        union
        SELECT parents.id, parents.pid, parents.category FROM mortgage_staging_v1.s_sales_company_orgs as parents INNER JOIN child ON parents.id = child.pid
        )
        SELECT
            child.id AS root_id
        FROM
        ...
        await db.execute(
            utils.gen_insert_sql(
                "mortgage_staging_v1.c_archive_files", {**item, "s_sales_company_org_id": parents["root_id"]}
            )
        )
    ...
    for old_file_info in old_files_info:
        ...
        c_archive_file = await db.fetch_one(
            f"SELECT * FROM mortgage_staging_v1.c_archive_files WHERE old_id = {old_file_info['old_record_id']};"
        )
        owner_id = None
        if c_archive_file:
    ...
    for data in new_data:
        await db.execute(utils.gen_insert_sql("mortgage_staging_v1.c_archive_uploaded_files", data))
