...
async def check_s_sales_person_with_email(db: DB, email: str):
    sql = f"SELECT CONVERT(id,CHAR) AS id,  status, DATE_FORMAT(failed_first_at, '%Y-%m-%d %H:%i:%S') as failed_first_at, failed_time, hashed_pwd FROM s_sales_persons WHERE email = '{email}';"
